...
double randomizedAverageSimple(word seed, int *v, size_t n, int a, int b) {
    auto x = seed;
    word sum = 0;
    for (auto i = 0; i < n; ++i) {
        x = A * x + B;
...
double randomizedAverageOmp(word seed, int *v, size_t n, int a, int b) {
...
#pragma omp parallel shared(partialSums, T)
    ...
    return (double) partialSums[0].value / n;
}


double randomizedAverageCpp(word seed, int *v, size_t n, int a, int b) {
